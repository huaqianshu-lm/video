...
- 仓库规则已按“核心 `CLAUDE.md`、专项 Skill、稳定 `docs/`、本地 `drafts/` 与 `notes/`”完成分层；四个项目内 Skill、通用视频模板和受跟踪 MVP 能力清单已创建并通过适用验证。仓库资料与代码分离的四阶段计划已保存到 `drafts/REPOSITORY-CLEANUP-FOUR-PHASE-PLAN.md`，第二阶段 Git 索引清理、第三阶段历史重写和现有远端分支强制更新已完成，第四阶段本地验证已完成，独立私有输入源已配置，真实远程 Smoke Render 等待 `vscode` Gate 3 人工确认后执行。
...
- 两个 GitHub Actions 渲染 Workflow 已改为接收独立输入包 URL／SHA-256，在 Runner 临时工作区恢复资料并生成临时 Remotion 入口；`vscode` 输入包已发布到独立私有仓库 Release，主仓库 `RENDER_INPUT_TOKEN` Secret 和本地远程配置均已通过诊断，真实 Smoke Render 等待 Gate 3。
...
- `vscode` 已重新执行 Remotion Agent，生成并校验 `remotion-alignment.json`，当前进入 `gate-3 / waiting`；远程输入包位于独立仓库 `huaqianshu-lm/video-render-inputs` 的 Release `vscode-input-v1`，未进入能力仓库。
...
- `vscode` 已完成 Remotion 重新校验，当前停在 Gate 3；需要人工对照 Visual Prototype、`remotion-alignment.json` 和 Studio 画面后确认。
...
1. 由用户完成 `vscode` Gate 3 人工确认；通过后提交真实 GitHub Actions Smoke Render，检查代表帧、字体、资源和音轨。
2. 在 Web UI 导入实际新视频原文件并跑通 Source 到 Gate 2；默认使用项目内 Codex CLI 适配器，执行失败时通过持久化 Job 重试。
3. 由用户确认新增 33 个视频的 Gate 2；通过后按四类批次逐段推进，历史完成视频不再回溯。
4. 继续按 `todo.md` 统一改造 Web UI 的其他异步／业务按钮；Smoke Render 一键交付已完成，完整 Render 继续保持 Smoke 检查后的独立入口。
