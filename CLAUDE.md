...
- 已标记为 `completed` 的视频永久只读。一旦视频进入该状态，任何后续任务、Agent、Harness、Web UI、批处理、渲染、状态刷新、校验、修复、规则迁移或其他操作都不得修改该视频的内容、生产资料、Remotion 配置和代码、音视频／字幕／Timeline／Manifest、Gate／审核记录、状态、阶段、进度、尝试次数、Job／任务记录及其他相关产物；只允许只读检查和展示。若必须产生新版本，只能创建新的视频标识或独立副本，不得回写原视频。
...
- 修改阶段顺序、调用入口、任务包／执行器、产物或状态契约、校验及下游消费方式前，先列出受影响的发起入口、单条与批量路径、生产者、消费者、旧路径、文档和测试，并明确新旧契约及验收方式；只核对本次变更实际涉及的环节。
- 迁移可以分步实施，但每一步都必须保持可用且约定一致。旧路径要么继续受支持并经验证，要么明确停用；不得依赖未声明的回退、过期临时产物或仅靠局部校验掩盖新旧流程不一致。
- 只有任务说明、实现、校验、使用入口和适用文档／测试均已同步，并从实际入口走通受影响的完整路径，才能声明迁移完成。无法完成或无法验证时，在 `ROADMAP.md` 标记“迁移中”或“阻塞”，写明剩余环节、影响和恢复步骤；不得把局部可用写成整体完成。
