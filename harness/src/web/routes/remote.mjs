...
import { assertRemotionTaskStageReady, completeRemotionTask, getRemotionTask, listRemotionTasks, recoverInterruptedRemotionTasks, remotionTaskForView, retryRemotionTask, startRemotionTask } from "../../remotion-tasks.mjs";
...
export function createRemoteRoutes({ runtime, diagnose = diagnoseGitHubActions } = {}) {
  return async function handleRemote({ request, response, pathname, search = "" }) {
    ...
    if (pathname === "/api/remotion-tasks" && request.method === "GET") { sendJson(response, 200, { tasks: listRemotionTasks().map(remotionTaskForView) }); return true; }
