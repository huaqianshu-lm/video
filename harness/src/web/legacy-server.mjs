...
async function serveBatchApi(response, request, pathname, runtime) {
...
async function serveRemotionTaskApi(response, request, pathname, runtime) {
...
async function serveAgentJobApi(response, request, pathname, runtime) {
...
async function serveAction(response, request, pathname, runtime) {
  ...
  if (action === "find-historical") {
    ...
    try {
      ...
      const candidates = await runtime.remoteJobMonitor.findHistorical({ slug, stage });
  ...
  if (action === "adopt-historical") {
    ...
    try {
      requireGitHubActionsConfig();
      const job = await runtime.remoteJobMonitor.adoptHistorical({ slug, stage, runId: body.runId });
  ...
  if (action === "remote-run") {
    ...
    try {
      ...
      const job = runtime.remoteJobMonitor.submit({ slug, stage });
  ...
  switch (action) {
    ...
    case "reject":
      ...
      if (result.stage === "gate-3" && result.returnTo === "remotion") {
        ...
        if (task.status !== "in-progress") runtime.queueRemotionTask(task.id);
        responseStatus = 202;
      }
      break;
    case "retry":
...
async function handleRequest(request, response, host, runtime, diagnose) {
  ...
  if (requestUrl.pathname.startsWith("/api/agent-jobs") && await serveAgentJobApi(response, request, requestUrl.pathname, runtime)) {
    return;
  }
  if (requestUrl.pathname.startsWith("/api/remotion-tasks") && await serveRemotionTaskApi(response, request, requestUrl.pathname, runtime)) {
    return;
  }
  if (requestUrl.pathname.startsWith("/api/batches") && await serveBatchApi(response, request, requestUrl.pathname, runtime)) {
    return;
  }
  if (isAction && await serveAction(response, request, requestUrl.pathname, runtime)) {
    return;
  }
  ...
  if (requestUrl.pathname.startsWith("/api/") && await serveApi(response, requestUrl.pathname, requestUrl.search, runtime.remoteJobMonitor, diagnose)) {
    return;
  }
