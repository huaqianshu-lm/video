...
export function createBatchView({ elements, api, onRefreshProjects, getActiveProject, onOpenProject } = {}) {
  let mounted = false;
  ...
  function itemCard(batch, item) {
    ...
    return `<li class="batch-item"><span><strong>${escapeHtml(item.slug)}</strong><small>${escapeHtml(message ?? "")}</small></span><span class="batch-item-actions"><span class="stage-status">${escapeHtml(labelFor(status, batchItemStatusLabels))}</span>${action}</span></li>`;
  }
  ...
  function mount() {
    ...
    elements.batchList.addEventListener("click", (event) => { const button = event.target.closest("[data-batch-action]"); if (button) void action(button.dataset.batchAction, button.dataset.batchId, button.dataset.slug); });
    elements.taskList.addEventListener("click", (event) => { const button = event.target.closest("[data-remotion-task-action]"); if (button) void taskAction(button.dataset.remotionTaskAction, button.dataset.taskId); });
    void refresh();
  }
  return { mount, unmount() { mounted = false; }, refresh, create: async (type, slugs) => { await api.createBatch({ type, slugs }); await refresh(); await onRefreshProjects?.(); } };
}
