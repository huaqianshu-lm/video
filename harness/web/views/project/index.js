...
export function createProjectView({ elements, api, polling, onBack, onRefreshDashboard } = {}) {
  ...
  function render(workspace) {
    ...
    [...elements.container.querySelectorAll(".jobs-section:last-of-type .stage-status")].forEach((node, index) => {
      const status = workspace.agentJobs?.[index]?.status;
      if (status) node.classList.add("status", `status-${status}`);
    });
    bindEvents(project);
  }
