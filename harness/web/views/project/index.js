...
function stageItem(stage) {
  ...
  return `<li class="stage-item stage-${escapeHtml(stage.status)}"><div class="stage-marker">${stage.order + 1}</div><div class="stage-content"><div class="stage-heading"><div><h3>${escapeHtml(stage.label ?? stage.stage)}</h3><p>${escapeHtml(stage.objective)}</p></div><span class="stage-status">${escapeHtml(labelFor(stage.status, stageStatusLabels))}</span></div><div class="stage-meta"><span>${escapeHtml(artifacts)}</span>${stage.requiresApproval ? "<span>需要人工确认</span>" : ""}${stage.review ? `<span>审查：${escapeHtml(stage.review.decision === "approved" ? "已通过" : "已驳回")}</span>` : ""}</div></div></li>`;
}
...
export function createProjectView({ elements, api, polling, onBack, onRefreshDashboard } = {}) {
  ...
  function actionControls(project, task, activeJob, continuousBatch) {
    ...
    if (project.next.action === "run-stage" && ["smoke-render", "render"].includes(project.currentStage)) buttons.push(activeJob?.stage === project.currentStage ? `<button class="button button-primary" type="button" disabled>远程任务执行中</button>` : `<button class="button button-primary" type="button" data-action="remote-run">提交远程任务</button>`);
  ...
  function unmount() { mounted = false; requestNumber += 1; currentSlug = null; currentWorkspace = null; polling.stop("project"); polling.stop("remotion-task"); }
